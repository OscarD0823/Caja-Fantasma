...
- Sincronización bidireccional PC–Android sin Firebase ni nube personal: Windows muestra por separado la IP, el puerto y un código de seis números. Android permite elegir explícitamente **PC → Celular** o **Celular → PC** para reemplazar una copia de forma manual. Además, el modo opcional **Sincronización en vivo** refleja automáticamente puntos, cajas, rondas, personajes, estadísticas y módulos Brillantes cuando está activado en ambos equipos; en Android continúa mediante un servicio nativo aunque se use otra aplicación. Una notificación silenciosa obligatoria muestra si el PC está conectado y desaparece al desactivar el modo. La revisión compartida combina registros desfasados y rechaza que un estado vacío borre un historial existente. Android ofrece cuatro bloques para la IP, un campo independiente para el puerto y admite pegar la dirección completa. El programa rechaza direcciones públicas y nunca copia el modo administrador ni los ajustes propios de la ventana.
