...
- Contador flotante movible, siempre visible, escalable entre 20 % y 150 % y con posición recordada. Toda su configuración se concentra en la pestaña Caja; Visión queda reservada para las ruedas publicadas. El área transparente de la Ballena usa una escala independiente de 20 % a 100 % y nunca supera el ancho de la ventana. El tiempo del Riftwalker puede conservar tamaño y estilo propios o esconderse para dejar únicamente al jefe y su rayo de progreso. La Ballena también puede ocultarse por completo sin desactivar el contador principal. Puede usar la forma automática propia de cada evento o un diseño rectangular, cuadrado, vertical o redondo; el reloj principal admite lectura digital, compacta o mediante anillo, y el nombre se muestra en español, inglés o con un texto personalizado. Funciona sobre juegos en ventana o pantalla completa sin bordes; Windows no permite garantizar superposición sobre pantalla completa exclusiva.
...
- Filtros por estilo de arma o pieza de armadura y por sistema de origen. Todos los módulos progresan desde Nivel 1 hasta Nivel 17; la versión especial se muestra como Brillante. Hora punta con Estrella descendente está incluida como combinación exacta y siguen disponibles los nombres personalizados para incorporaciones futuras.
- Respaldo e importación JSON. Los datos personales se guardan localmente y solo viajan a otro dispositivo cuando la persona activa la sincronización directa PC–Android.
- Sincronización bidireccional PC–Android sin Firebase ni nube personal: con ambas aplicaciones abiertas en una red local o mediante USB tethering, Windows muestra por separado la IP, el puerto y un código de seis números. Android permite elegir explícitamente **PC → Celular** o **Celular → PC** para intercambiar puntos, cajas, rondas, personajes, historial y módulos Brillantes sin depender del reloj de los dispositivos. El sondeo automático solo comprueba la conexión y retransmite al teléfono el catálogo público vigente. Android ofrece cuatro bloques para la IP, un campo independiente para el puerto y admite pegar la dirección completa. El programa rechaza direcciones públicas y nunca copia el modo administrador ni los ajustes propios de la ventana.
- Interfaz principal seleccionable en español o inglés; el idioma queda guardado de manera independiente en cada dispositivo.
...
La APK reutiliza la interfaz y el núcleo Tauri, conserva sus datos personales en el teléfono y recibe la configuración pública desde GitHub o desde el PC conectado. Puede intercambiar los datos personales directamente con el PC, sin Firebase, nube ni cuentas, mientras ambas aplicaciones estén abiertas en la misma red local o mediante USB tethering. La implementación, sus límites y las pruebas están documentados en [`docs/ANDROID-Y-SINCRONIZACION.md`](docs/ANDROID-Y-SINCRONIZACION.md).
