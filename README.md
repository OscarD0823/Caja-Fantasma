...
- Sincronización bidireccional PC–Android sin Firebase ni nube personal: con ambas aplicaciones abiertas en una red local o mediante USB tethering, Windows muestra por separado la IP, el puerto y un código de seis números. Android permite elegir explícitamente **PC → Celular** o **Celular → PC** para reemplazar una copia de forma manual. Además, el modo opcional **Sincronización en vivo** refleja automáticamente puntos, cajas, rondas, personajes, estadísticas y módulos Brillantes cuando está activado en ambos equipos; usa una revisión compartida y rechaza automáticamente que un estado vacío borre un historial existente. Si solo uno de los dispositivos conserva registros durante la primera conexión, esa copia se adopta en ambos. Antes de una reducción del historial también se conserva una copia local de seguridad. Android ofrece cuatro bloques para la IP, un campo independiente para el puerto y admite pegar la dirección completa. El programa rechaza direcciones públicas y nunca copia el modo administrador ni los ajustes propios de la ventana.
